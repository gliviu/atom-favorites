...
function refreshFavoritesMenu(){
  ...
  for (let favConfig of favs) {
    const {isTopic, topic, fav} = common.parseFavoriteConfig(favConfig)
    if(isTopic){
...
function refreshKeyBindings(){
  ...
  for (let favConfig of favs) {
    const {hasKeymap, keymap, fav, isTopic} = common.parseFavoriteConfig(favConfig)
    if(isTopic){
      continue
    }
    const favHash = hash(fav)
...
function refreshFavorites(){
  ...
  const favs = atom.config.get(FAVORITE_FILES_CONFIG_KEY) || []
  for (const favConfig of favs) {
    const {isTopic, fav} = common.parseFavoriteConfig(favConfig)
    if(isTopic){
      continue
    }
...
function removeFavorite(path){
  ...
  const newFavs = favs
  .map(favConfig => common.parseFavoriteConfig(favConfig).fav)
  .filter(fav =>{
    if(!fav){
      return false
    }
