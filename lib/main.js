...
export default {
  activate() {
    ...
    let favs = atom.config.get(FAVORITE_FILES_CONFIG_KEY)
    if(!favs){
      favs = []
...
function refreshFavoritesMenu(favoritesInfo){
  ...
  favoriteMenuItems.push({ label: 'Configure', command: OPEN_SETTINGS_COMMAND_KEY })
...
function createSettingsCommand(){
  atom.commands.add('atom-workspace', OPEN_SETTINGS_COMMAND_KEY, ()=>{
