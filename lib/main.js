...
function refreshFavoritesMenu(){
  ...
  for (let favConfigLine of favs) {
    ...
    const label = computeLabel(fav, isRenamed, newName)
...
function refreshFavorites(){
  ...
  for (const favConfigLine of favs) {
    const favConfig = common.parseFavoriteConfig(favConfigLine)
    const {isTopic, isRenamed, newName, fav} = favConfig
    if(isTopic){
      continue
    }
    if(!fav){
...
function computeLabel(fav, isRenamed, newName){
  let folderIcon = '>'
  if(ISLIN || ISMAC){
    folderIcon = '📁'
  }
  const name = isRenamed?newName:pathUtils.basename(fav)
  return fs.existsSync(fav) && isFolder(fav)
